...
Scene::Scene(const Napi::CallbackInfo &info) : Common(info.This(), "Scene") {
	NAPI_ENV;
	super(info);
	...
	_physWorld =
	    ALIGNED_NEW(btDiscreteDynamicsWorld, _physDispatcher, _physBroadphase, _physSolver, _physConfig);
...
JS_IMPLEMENT_SETTER(Scene, gravity) {
	THIS_CHECK;
	SETTER_VEC3_ARG;
...
JS_IMPLEMENT_METHOD(Scene, update) {
	THIS_CHECK;
	LET_FLOAT_ARG(0, dt);
...
JS_IMPLEMENT_METHOD(Scene, trace) {
	THIS_CHECK;
	REQ_VEC3_ARG(0, f);
	REQ_VEC3_ARG(1, t);
...
JS_IMPLEMENT_METHOD(Scene, destroy) {
	THIS_CHECK;
	emit("destroy");
	_destroy();
	RET_UNDEFINED;
...
JS_IMPLEMENT_GETTER(Scene, isDestroyed) {
	THIS_CHECK;
