...
#define V3_GETTER(CLASS, NAME, CACHE)                                                                        \
	JS_IMPLEMENT_GETTER(CLASS, NAME) {                                                                       \
		THIS_CHECK;                                                                                          \
...
struct Common {
	Common(Napi::Value that, const char *name) : _asyncCtx(that.Env(), name) {
	...
	void emit(const char *name, int argc = 0, const Napi::Value *argv = nullptr) {
		DES_CHECK;
